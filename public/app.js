  ...
  screen.querySelectorAll('.nav-item[data-view]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const titles = {
        ...
        'admin-rondas':'Control de Rondas', 'admin-recalcular':'Recalcular Estados Financieros', 'admin-resultados':'Resultados',
  ...
  el.innerHTML = `
    <div style="padding:24px">
      ...
      <p style="font-size:.84rem;color:var(--text3);margin-bottom:20px;line-height:1.6">
        ...
        Conserva las decisiones originales. Útil cuando se cambian parámetros de industria o costos.
      </p>
      <button class="btn btn-primary" id="btnRecalcAction" onclick="doRecalcularYMostrar()">
        🔄 Ejecutar Recálculo de Todas las Rondas
      </button>
  ...
  window.doRecalcularYMostrar = async function() {
    ...
    try {
      ...
      if (rep) rep.innerHTML = `
        <div style="background:rgba(158,216,48,0.08);border:1px solid rgba(158,216,48,0.3);border-radius:var(--r);padding:16px 20px">
          <div style="font-weight:700;color:var(--accent3);margin-bottom:8px">✅ Recálculo completado</div>
          <div style="font-size:.84rem;color:var(--text2)">Rondas actualizadas: <strong>${ok}</strong></div>
          <div style="font-size:.78rem;color:var(--text3);margin-top:12px">Los estados financieros han sido actualizados. Verifica en Resultados.</div>
        </div>`;
    } catch(e) {
