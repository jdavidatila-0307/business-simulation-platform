  ...
  screen.querySelectorAll('.nav-item[data-view]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const titles = {
        ...
        'admin-rondas':'Control de Rondas', 'admin-recalcular':'Recalcular EF', 'admin-resultados':'Resultados',
      ...
      if (btn.dataset.view === 'admin-recalcular') loadAdminRecalcular();
...
async function loadAdminRecalcular() {
  ...
  el.innerHTML = '<div style="padding:24px">'
    + '<h2 style="font-size:1.1rem;font-weight:700;margin-bottom:8px;color:var(--accent3)">⚡ Recalcular Estados Financieros</h2>'
    + '<p style="font-size:.84rem;color:var(--text3);margin-bottom:20px;line-height:1.6">'
    + 'Re-ejecuta el motor contable para <strong>todas las rondas ejecutadas</strong> con los parámetros actuales.'
    + ' Conserva las decisiones originales. Corrige descuadres en estados financieros.</p>'
    + '<button class="btn btn-primary" id="btnRecalcAction" onclick="doRecalcularYMostrar()">'
    + '🔄 Ejecutar Recálculo de Todas las Rondas</button>'
    + '<div id="recalcularReporte" style="margin-top:24px"></div>'
    + '</div>';
  window.doRecalcularYMostrar = async function() {
    const btn = document.getElementById('btnRecalcAction');
    const rep = document.getElementById('recalcularReporte');
    if (btn) { btn.disabled = true; btn.textContent = '⏳ Recalculando...'; }
    try {
      const r = await api('POST', '/admin/recalcular-balance');
      const ok = r.rondas || r.actualizadas || r.ok || 0;
      if (rep) rep.innerHTML = '<div style="background:rgba(158,216,48,0.08);border:1px solid rgba(158,216,48,0.3);border-radius:var(--r);padding:16px 20px">'
        + '<div style="font-weight:700;color:var(--accent3);margin-bottom:8px">✅ Recálculo completado</div>'
        + '<div style="font-size:.84rem">Rondas actualizadas: <strong>' + ok + '</strong></div>'
        + '<div style="font-size:.78rem;color:var(--text3);margin-top:8px">Verifica los resultados en el panel de Resultados.</div>'
        + '</div>';
    } catch(e) {
      if (rep) rep.innerHTML = '<div style="color:var(--accent4);padding:12px">Error: ' + e.message + '</div>';
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = '🔄 Ejecutar Recálculo de Todas las Rondas'; }
    }
  };
}
...
async function loadAdminRondasDirect() {
  ...
  try {
    ...
    const rows = hist.map(h => `<tr>
      ...
      <td style="text-align:center"><span class="badge ${h.estado==='calculada'||h.estado==='simulated'?'badge-ok':'badge-warn'}">${h.estado}</span></td>
    ...
    el.innerHTML = '<div class="table-wrap"><table>'
      + '<thead><tr><th style="text-align:center">Trimestre</th><th style="text-align:center">Estado</th>'
      + '<th style="text-align:center">Decisiones enviadas</th><th style="text-align:center">Ejecutada</th>'
